...
if (!isset($_SESSION)) {
    ...
    }elseif(isset($_SESSION['Technical'])) {
        $sql = "SELECT * FROM `users` WHERE `role` = '{$_SESSION['Technical']}'";
    }elseif(isset($_SESSION['Supported'])) {
        $sql = "SELECT * FROM `users` WHERE `role` = '{$_SESSION['Supported']}'";
    }elseif(isset($_SESSION['Expert'])) {
        $sql = "SELECT * FROM `users` WHERE `role` = '{$_SESSION['Expert']}'";
    }elseif(isset($_SESSION['Graphic'])) {
        $sql = "SELECT * FROM `users` WHERE `role` = '{$_SESSION['Graphic']}'";
    }else {
        header("Location: login.php");
        exit();
    }
//    if(!isset($_SESSION['Admin']) || !isset($_SESSION['Technical']) || !isset($_SESSION['Supported']) || !isset($_SESSION['Expert'])) {
//        header("Location: login.php");
//        exit();
//    }
}
    ...
    <div class="container-fluid-full">
        <div class="row-fluid">
            ...
            <div id="content" class="span10">
                <a href="<?= BASE_DOMAIN; ?>/backend/dashboard.php" class="btn btn-success">بازگشت</a>
                <div class="page-header">ویرایش پروفایل</div>
                <div class="container">
                    <form class="form-horizontal" action="" method="post">
                        ...
                        <?php
                            if(isset($_SESSION['Admin'])) {
                                ...
                                ?>
                                <label class="control-label" for="role">سطح دسترسی</label>
                                <input type="text" name="role" id="role" class="controls" readonly="readonly" value="<?= $result['role']; ?>">
                                <?php
                            }
                        ?>
                        <div class="control-group">
