...
$s = "SELECT problems.description,problem_project.problem_id,projects.project_name FROM `projects` JOIN `problem_project` JOIN
                               `problems`
                               WHERE problem_project.problem_id = '{$id}'
                               || problem_project.project_id = '{$id}'";
//var_dump($s);exit();

//$p = $project->readProject($s);

$domain = new dbProblem("support", "localhost", "root", "");

$p = $domain->readProblem($s);
