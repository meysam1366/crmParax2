...
require_once '../core/core.php';
    ...
    if(isset($_SESSION['Admin'])) {
        ...
        if (isset($_POST['login'])) {
            ...
            $result = $user->checkLogin('users', $username);
    ...
    //        exit();
            ...
            if($result['username'] == $username && $result['password'] == $password) {
                if ($result['role'] == 'Admin') {
                    $_SESSION[$result['role']] = $result['role'];
                    header("Location: dashboard.php");
                    exit();
                }elseif ($result['role'] == 'Technical') {
                    $_SESSION[$result['role']] = $result['role'];
                    header("Location: technical.php");
                    exit();
                }elseif ($result['role'] == 'Supported') {
                    $_SESSION[$result['role']] = $result['role'];
                    header("Location: supported.php");
                    exit();
                }elseif ($result['role'] == 'Expert') {
                    $_SESSION[$result['role']] = $result['role'];
                    header("Location: expert.php");
                    exit();
                }elseif ($result['role'] == 'Graphic') {
                    $_SESSION[$result['role']] = $result['role'];
                    header("Location: graphic.php");
                    exit();
                }
